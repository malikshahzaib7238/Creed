...
const MobileNavbar = ({ navLinks = [] }) => {
    ...
    // The logic inside the hooks remains the same as it's already dynamic
    const moveIndicatorTo = (button) => {
        ...
        setTimeout(() => {
            ...
            if (navBarRect.width > 0) {
                const offset = buttonRect.left - navBarRect.left + (buttonRect.width / 2) - (indicatorRef.current.offsetWidth / 2);
    ...
    useEffect(() => {
        const updateActiveState = () => {
            ...
            buttonRefs.current.forEach(btn => {
                ...
                // Improved logic for finding the active button
                if (currentHash && btnPath === `/${currentHash}` || currentHash && btnPath == `/#top`) {
                    activeButton = btn;
            ...
            if (!activeButton && currentPath === '/') {
                activeButton = buttonRefs.current.find(btn => btn && btn.getAttribute('href').endsWith('#top'));
            }
    ...
    return (
        <nav className="mobile-nav-bar" ref={navBarRef}>
            <div id="nav-indicator" className="nav-indicator spotlight" ref={indicatorRef}></div>
